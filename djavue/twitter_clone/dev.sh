...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=twitter_clone.example.com
...
workon twitter_clone  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development twitter_clone npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop twitter_clone
    docker rm twitter_clone
    docker run --name twitter_clone -d --env-file /home/ubuntu/twitter_clone.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/twitter_clone:/dkdata \
        twitter_clone start_web.sh
}
...
echo_green "Welcome to twitter_clone's dev env"
